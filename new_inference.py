...
def run_inference(args):
    ...
    if args.direction == "c2e":
        ...
        if args.phase == "test_unknown_content":
            # --- CHECK PATH TỒN TẠI ---
            if not args.chinese_train_dir or not os.path.exists(args.chinese_train_dir):
                print(f"❌ LỖI: Đường dẫn Chinese Train không tồn tại: {args.chinese_train_dir}")
                return
        ...
        if args.complexity == "all":
            try:
                # Lấy font đầu tiên làm mẫu
                fonts = os.listdir(style_search_root)
                if fonts:
                    first_font = fonts[0]
                    candidate_glyph_set = set(os.listdir(os.path.join(style_search_root, first_font)))
            except Exception as e: 
                print(f"Lỗi đọc complexity 'all': {e}")
        else:
            ...
            for eng_path in target_images:
                ...
                if os.path.exists(content_path):
                    samples.append({
                        ...
                        "font_name": font_name, # Đã sửa key cho khớp
                        "glyph_name": glyph_name # Đã sửa key cho khớp
                    })
    ...
    # ==========================================
        ...
        target_images = collect_files(args.chinese_dir)
        ...
        for chi_path in target_images:
            ...
            if style_file and os.path.exists(content_path):
                samples.append({
                    ...
                    "font_name": font_name, # Đã sửa key
                    "glyph_name": glyph_name # Đã sửa key
                })
...
def parse_args():
    ...
    parser.add_argument('--chinese_train_dir', type=str, default=None)
    parser.add_argument('--direction', type=str, default='c2e')
    parser.add_argument('--phase', type=str, default='test_unknown_style')
    parser.add_argument('--complexity', type=str, default='hard')
